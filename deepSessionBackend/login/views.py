...
from .models import NewOTP, NewUser
...
class SendOTPView(APIView):
    def post(self, request):
        ...
        user, _ = NewUser.objects.get_or_create(email=email)

        # Generate OTP
        code = str(random.randint(100000, 999999))
        expires_at = timezone.now() + datetime.timedelta(minutes=10)

        NewOTP.objects.create(user=user, code=code, expires_at=expires_at)
...
class VerifyOTPView(APIView):
    def post(self, request):
        ...
        try:
            ...
            otp = NewOTP.objects.filter(user=user, code=otp_code, is_used=False).last()
